...
$query_barang = mysqli_query($conn, "SELECT * FROM items WHERE id = '$id'");
...
<head>
    ...
    <title>Edit Barang Lab - <?= htmlspecialchars($barang['nama']) ?></title>
...
<body>
    ...
    <div class="main-wrapper">
        <div class="page-header">
            <h2><i class="fa-solid fa-pen-to-square"></i> Edit Data Barang</h2>
            <p>Perbarui informasi detail aset atau logistik inventaris laboratorium.</p>
        </div>

        <div class="form-container">
            <form action="prose_edit_barang.php?id=<?= $barang['id'] ?>" method="POST" enctype="multipart/form-data">
                <div class="form-grid">
                    
                    <div class="form-group">
                        <label>Kode Barang</label>
                        <input type="text" name="kode" class="form-control" value="<?= htmlspecialchars($barang['kode']) ?>" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="id_lokasi" class="form-control" required>
                            ...
                            <?php while ($lok = mysqli_fetch_assoc($query_lokasi)): ?>
                                <option value="<?= $lok['id'] ?>" <?= $barang['id_lokasi'] == $lok['id'] ? 'selected' : '' ?>>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="stok" min="0" class="form-control" value="<?= htmlspecialchars($barang['stok']) ?>" required>
                    </div>

                    <div class="form-group">
                        <label>Satuan</label>
                        <input type="text" name="satuan" class="form-control" value="<?= htmlspecialchars($barang['satuan']) ?>" placeholder="Pcs / Unit / Botol" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="stok_minimum" min="0" class="form-control" value="<?= htmlspecialchars($barang['stok_minimum']) ?>" required>
                    </div>

                    <div class="form-group tf-full form-group-full">
                        <label>Foto Barang</label>
                        <?php if (!empty($barang['foto_barang'])): ?>
                            <div>
                                <div class="current-photo-info">
                                    <i class="fa-solid fa-image"></i> Berkas saat ini: <em><?= htmlspecialchars($barang['foto_barang']) ?></em>
                                </div>
                            </div>
                        <?php endif; ?>
                        <input type="file" name="foto_barang" accept="image/*" class="form-control">
                    </div>

                    <div class="form-group form-group-full">
                        <label>Keterangan Tambahan</label>
                        <textarea name="keterangan" rows="3" class="form-control" placeholder="Tulis catatan penunjang di sini..."><?= htmlspecialchars($barang['keterangan']) ?></textarea>
                    </div>
                ...
                <div class="form-actions">
                    ...
                    <button type="submit" name="update" class="btn btn-primary"><i class="fa-solid fa-floppy-disk"></i> Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
